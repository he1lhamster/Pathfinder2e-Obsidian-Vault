---
actions: '[two-actions]'
component:
- Somatic
- Verbal
duration: 1 minute
heighten: '+2'
heighten_level: 2, 4, 6, 8, 10
id: '1018'
level: '2'
name: Thundering Dominance
range: 30 feet
rarity: Common
school: Enchantment
source: '[[DATABASE/source/Secrets of Magic|Secrets of Magic]]'
target: yourcompanionoreidolon
tradition:
- Occult
- Primal
trait:
- '[[DATABASE/trait/Enchantment|Enchantment]]'
- '[[DATABASE/trait/Mental|Mental]]'
type: Spell

---
# Thundering Dominance<span class="item-type">Spell 2</span>

<span class="item-trait">Enchantment</span><span class="item-trait">Mental</span>
**Source** [[DATABASE/source/Secrets of Magic|Secrets of Magic]] 
**Traditions** occult, primal
**Cast** <span class="action-icon">2</span> somatic, verbal
**Range** 30 feet; **Targets** your companion or eidolon
**Duration** 1 minute

---
You shroud the target in a powerful predatory aura and amplify its vocalizations. It gains a +1 status bonus to [[DATABASE/skill/Intimidation|Intimidation]] checks. Once during this spell's duration, it can take the Thundering Roar action.
 **Thundering Roar** <span class="action-icon">1</span> (auditory, concentrate, emotion, enchantment, fear, flourish, mental, sonic) The target unleashes a powerful vocalization. Each enemy in a 10- foot emanation takes 4d8 sonic damage with a basic Will save against your spell DC. A creature that fails its Will save is also [[DATABASE/condition/Frightened|frightened 1]].

---
**Heightened (+2)** The status bonus to Intimidation checks increases by 1 (to a maximum of a +4 status bonus for an 8th-level spell), and the damage from the roar increases by 2d8.