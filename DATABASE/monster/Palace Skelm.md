---
ac: '27'
alignment: LE
charisma: '+5'
constitution: '+3'
creature_ability:
- Change Shape
- Corrupt Speech
- Incite Violence
creature_family: '[[DATABASE/monsterfamily/Skelm|Skelm]]'
dexterity: '+4'
fortitude: '+17'
hp: '155'
id: '1303'
intelligence: '+4'
land_speed: '25'
language:
- '[[DATABASE/language/Aklo|Aklo]]'
- '[[DATABASE/language/Common|Common]] ; [[DATABASE/spell/Tongues|tongues]]'
level: '8'
max_speed: '25'
name: Palace Skelm
perception: '+15'
rarity: Common
reflex: '+16'
sense:
- '[[DATABASE/monsterability/Scent|scent]] (imprecise) 30 feet'
size: Medium
skill:
- '[[DATABASE/skill/Athletics|Athletics]] +18'
- '[[DATABASE/skill/Deception|Deception]] +21'
- '[[DATABASE/skill/Diplomacy|Diplomacy]] +17'
- '[[DATABASE/skill/Intimidation|Intimidation]] +17'
- '[[DATABASE/skill/Occultism|Occultism]] +16'
- '[[DATABASE/skill/Society|Society]] +16'
- '[[DATABASE/skill/Stealth|Stealth]] +16'
- '[[DATABASE/skill/Thievery|Thievery]] +16'
source: '[[DATABASE/source/Bestiary 3|Bestiary 3]]'
speed:
- 25 feet
spell:
- '[[DATABASE/spell/Clairaudience|Clairaudience]]'
- '[[DATABASE/spell/Daze|Daze]]'
- '[[DATABASE/spell/Enthrall|Enthrall]]'
- '[[DATABASE/spell/Invisibility|Invisibility]]'
- '[[DATABASE/spell/Message|Message]]'
- '[[DATABASE/spell/Outcast''s Curse|Outcast''s Curse]]'
- '[[DATABASE/spell/Paranoia|Paranoia]]'
- '[[DATABASE/spell/Private Sanctum|PrivateSanctum]]'
- '[[DATABASE/spell/Suggestion|Suggestion]]'
- '[[DATABASE/spell/Tongues|Tongues]]'
strength: '+6'
strength_req: '6'
strongest_save:
- Fortitude
trait:
- '[[DATABASE/trait/Humanoid|Humanoid]]'
- '[[DATABASE/trait/Skelm|Skelm]]'
type: Creature
weakest_save:
- Will
weakness:
- '[[DATABASE/equipment/Cold Iron Weapon|cold iron]] 10'
will: '+15'
wisdom: '+3'

---
# Palace Skelm

Skelms crave power, and the palace skelms who stalk the halls where such power concentrates are among the most dangerous of their kind. They assume titles that allow them maximum freedom to punish or hurt others with minimal oversight: any vindictive guard captain, tyrannical viceroy, or needlessly cruel magistrate could be or become a palace skelm. They delight in the wealth and status of their positions, garbing themselves in ornate finery that reflects their station.
 Palace skelms ingratiate themselves with powerful individuals and gather followers by stoking fears of losing long-held or hard-earned power—especially power gained through illicit means. They undermine their enemies by encouraging competition, jealousy, and outright paranoia by way of magic and false messages. These skelms possess an unnatural ability to twist spoken words and worm their manipulative magic into others' speech, sowing confusion and hatred that might explode into violence.
 A political upheaval is a palace skelm's greatest fear and opportunity alike, upsetting the balance of power he has built but providing countless new opportunities to sow hatred and evil. This conflicting goal leads palace skelms to self-sabotage and undermine their own efforts more than any other skelm.
**Recall Knowledge - Humanoid ([[DATABASE/skill/Society|Society]])**: DC 24
**Unspecific Lore**: DC 22
**Specific Lore**: DC 19

# Palace Skelm<span class="item-type">Creature 8</span>

<span class="trait-alignment item-trait">LE</span><span class="trait-size item-trait">Medium</span><span class="item-trait">Humanoid</span><span class="item-trait">Skelm</span>
**Source** [[DATABASE/source/Bestiary 3|Bestiary 3]]
**Perception** +15; [[DATABASE/monsterability/Scent|scent]] (imprecise) 30 feet
**Languages** [[DATABASE/language/Aklo|Aklo]], [[DATABASE/language/Common|Common]]; [[DATABASE/spell/Tongues|tongues]]
**Skills** [[DATABASE/skill/Athletics|Athletics]] +18, [[DATABASE/skill/Deception|Deception]] +21, [[DATABASE/skill/Diplomacy|Diplomacy]] +17, [[DATABASE/skill/Intimidation|Intimidation]] +17, [[DATABASE/skill/Occultism|Occultism]] +16, [[DATABASE/skill/Society|Society]] +16, [[DATABASE/skill/Stealth|Stealth]] +16, [[DATABASE/skill/Thievery|Thievery]] +16
**Str** +6, **Dex** +4, **Con** +3, **Int** +4, **Wis** +3, **Cha** +5
**Items** signet ring

---
**AC** 27; **Fort** +17, **Ref** +16, **Will** +15; –2 to all saves vs. emotion effects
**HP** 155; **Weaknesses** [[DATABASE/equipment/Cold Iron Weapon|cold iron]] 10
<span class="in-box-ability">**Corrupt Speech <span class="action-icon">5</span> ** (auditory, illusion, linguistic, occult) **Trigger** The skelm hears a creature speak within 30 feet; **Effect** The skelm sows paranoia by putting treacherous words on another's lips. The skelm whispers up to 12 words and attempts a [[DATABASE/skill/Deception|Deception]] check against the Perception DC of a creature other than the triggering creature within 30 feet. </span><span class="in-box-ability">**Critical Success** The target hears the skelm's words as if they were spoken by the triggering creature. This can alter linguistic effects. The skelm also casts [[DATABASE/spell/Paranoia|paranoia]] or [[DATABASE/spell/Suggestion|suggestion]] on the target, if he likes. </span><span class="in-box-ability">**Success** As critical success, except the skelm can't cast _paranoia_ or _suggestion_. </span><span class="in-box-ability">**Failure** The target doesn't hear the skelm's words, and they have no effect. </span><span class="in-box-ability">**Critical Failure** The target hears the skelm speak the words.</span>

---
**Speed** 25 feet
<span class="in-box-ability">**Melee** <span class="action-icon">1</span> fist +20 [+16/+12] (agile, magical), **Damage** 2d8+9 bludgeoning plus [[DATABASE/monsterability/Grab|Grab]]</span><span class="in-box-ability">**Melee** <span class="action-icon">1</span> antler +20 [+15/+10] (magical), **Damage** 2d12+9 piercing plus [[DATABASE/monsterability/Knockdown|Knockdown]]</span>**Occult Innate Spells** DC 26; **4th** _[[DATABASE/spell/Clairaudience|clairaudience]]_, _[[DATABASE/spell/Enthrall|enthrall]]_, _[[DATABASE/spell/Outcast's Curse|outcast's curse]]_, _[[DATABASE/spell/Paranoia|paranoia]]_ (at will), _[[DATABASE/spell/Private Sanctum|private sanctum]]_, _[[DATABASE/spell/Suggestion|suggestion]]_; **2nd** _[[DATABASE/spell/Invisibility|invisibility]]_ (×3); **Cantrips** **(5th)** _[[DATABASE/spell/Daze|daze]]_, _[[DATABASE/spell/Message|message]]_; **Constant** **(5th)** _[[DATABASE/spell/Tongues|tongues]]_
**Rituals** DC 26; **2nd** _[[DATABASE/ritual/Inveigle|inveigle]]_
<span class="in-box-ability">**[[DATABASE/monsterability/Change Shape|Change Shape]]** <span class="action-icon">1</span> (concentrate, occult, polymorph, transmutation) The palace skelm can take on the appearance of any Medium male humanoid. This doesn't change his Speed or his attack and damage bonuses with his Strikes but might change the damage type his Strikes deal.</span><span class="in-box-ability">**Incite Violence** <span class="action-icon">2</span> (emotion, enchantment, occult, mental) **Frequency** once per day; **Effect** The skelm gives an impassioned speech calling for his followers to act upon their convictions. Each creature within 30 feet must attempt a DC 24 Will save. 
**Critical Success** The creature can immediately Strike an adjacent creature of its choosing. 
**Success** The creature is unaffected. 
**Failure** The creature immediately Strikes an adjacent creature; if multiple creatures are adjacent, the skelm chooses the target. If no creatures are adjacent, the creature is [[DATABASE/condition/Flat-Footed|flat-footed]] and [[DATABASE/condition/Fascinated|fascinated]] with the skelm until the start of its next turn.</span>