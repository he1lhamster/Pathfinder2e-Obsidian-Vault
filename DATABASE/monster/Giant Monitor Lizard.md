---
ac: '18'
alignment: N
charisma: '-2'
constitution: '+3'
creature_ability:
- Eflect
- Gnashing Grip
- Lurching Charge
- Monitor Lizard Venom
creature_family: '[[DATABASE/monsterfamily/Lizard|Lizard]]'
dexterity: '+2'
fortitude: '+9'
hp: '30'
id: '289'
intelligence: '-4'
land_speed: '30'
level: '2'
max_speed: '30'
name: Giant Monitor Lizard
perception: '+7'
rarity: Common
reflex: '+8'
sense:
- low-light vision
- scent (imprecise) 30 feet
size: Medium
skill:
- '[[DATABASE/skill/Acrobatics|Acrobatics]] +6'
- '[[DATABASE/skill/Athletics|Athletics]] +9'
- '[[DATABASE/skill/Stealth|Stealth]] +6'
source: '[[DATABASE/source/Bestiary|Bestiary]]'
speed:
- 30 feet
- swim 30 feet
strength: '+3'
strength_req: '3'
strongest_save:
- Fortitude
swim_speed: '30'
trait:
- '[[DATABASE/trait/Animal|Animal]]'
type: Creature
vision: Low-light vision
weakest_save:
- Will
will: '+5'
wisdom: '+1'

---
# Giant Monitor Lizard

Large and seemingly ponderous, a monitor lizard is a deceptively swift and ruthless predator. It ambushes its prey by rushing out from cover and biting the target with its powerful jaws. A giant monitor lizard’s saliva is venomous, allowing it to bring down prey larger than it can easily haul away in its jaws. Giant monitor lizards grow up to 14 feet long, including their long tails, and they weigh about 350 pounds. Their bodies are normally dark brown with patches of yellow or green. When nesting, a giant monitor lizard digs a deep burrow to hide in. The burrow serves as both a safe haven and a location from which the lizard can ambush larger prey such as deer, boars, or even humanoids. A giant monitor lizard can consume nearly its own body weight in a single meal, and its loosely articulated jaws allow it to swallow surprisingly large prey.
**Recall Knowledge - Animal ([[DATABASE/skill/Nature|Nature]])**: DC 16
**Unspecific Lore**: DC 14
**Specific Lore**: DC 11

# Giant Monitor Lizard<span class="item-type">Creature 2</span>

<span class="trait-alignment item-trait">N</span><span class="trait-size item-trait">Medium</span><span class="item-trait">Animal</span>
**Source** [[DATABASE/source/Bestiary|Bestiary]]
**Perception** +7; low-light vision, scent (imprecise) 30 feet
**Skills** [[DATABASE/skill/Acrobatics|Acrobatics]] +6, [[DATABASE/skill/Athletics|Athletics]] +9, [[DATABASE/skill/Stealth|Stealth]] +6
**Str** +3, **Dex** +2, **Con** +3, **Int** -4, **Wis** +1, **Cha** -2

---
**AC** 18; **Fort** +9, **Ref** +8, **Will** +5
**HP** 30
<span class="in-box-ability">**Gnashing Grip** <span class="action-icon">5</span> **Trigger** A creature grabbed by the giant monitor lizard’s jaws fails a check to Escape. **Eflect** The giant monitor lizard’s jaws deal 1d6 piercing damage and the triggering creature is exposed to monitor lizard venom.</span>

---
**Speed** 30 feet, swim 30 feet
<span class="in-box-ability">**Melee** <span class="action-icon">1</span> jaws +11 [+6/+1], **Damage** 1d10+3 piercing plus Grab and monitor lizard venom</span><span class="in-box-ability">**Lurching Charge** <span class="action-icon">2</span> The giant monitor lizard Strides twice and then makes a jaws Strike. If the lizard moved at least 20 feet away from its starting position, it gains a +2 circumstance bonus to this attack roll.</span><span class="in-box-ability">**Monitor Lizard Venom** (poison) **Saving Throw** DC 17 Fortitude; **Maximum Duration** 6 rounds; **Stage 1** [[DATABASE/condition/Enfeebled|enfeebled 1]] (1 round); **Stage 2** 1d6 poison damage and [[DATABASE/condition/Enfeebled|enfeebled 2]] (1 round)</span>