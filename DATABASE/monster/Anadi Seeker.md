---
ac: '15'
alignment: CG
all_resistance: null
burrow_speed: null
charisma: '+0'
climb_speed: '25'
constitution: '-1'
creature_ability:
- Anadi Venom
- Change Shape
- Spin Silk
creature_family: '[[DATABASE/monsterfamily/Anadi|Anadi]]'
description: 'Anadi seekers are knowledge seekers who often brave the outside world.<br/><br/><b><u>Recall
  Knowledge - Humanoid</u> ( [[DATABASE/skill/Society|Society]] )</b>: DC 17<br/><b><u>Unspecific
  Lore</u></b>: DC 15<br/><b><u>Specific Lore</u></b>: DC 12'
dexterity: '+3'
element: null
fly_speed: null
fortitude: '+4'
hardness: null
hp: '20'
id: '1606'
immunity: null
intelligence: '+4'
land_speed: '25'
language:
- '[[DATABASE/language/Anadi|Anadi]]'
- '[[DATABASE/language/Common|Common]]'
level: '1'
max_speed: '25'
name: Anadi Seeker
perception: '+7'
rarity: Uncommon
reflex: '+10'
resistance: null
rus_type_level: null
school: null
sense: null
size: Medium
skill:
- '[[DATABASE/skill/Acrobatics|Acrobatics]] +6'
- '[[DATABASE/skill/Arcana|Arcana]] +7'
- '[[DATABASE/skill/Athletics|Athletics]] +4'
- '[[DATABASE/skill/Crafting|Crafting]] +7'
- '[[DATABASE/skill/Nature|Nature]] +5'
- '[[DATABASE/skill/Occultism|Occultism]] +7'
source: '[[DATABASE/source/Pathfinder 169. Kindled Magic|Pathfinder #169: Kindled
  Magic]]'
speed:
- 25 feet
- climb 25 feet
spell:
- '[[DATABASE/spell/Color Spray|Color Spray]]'
- '[[DATABASE/spell/Dancing Lights|DancingLights]]'
- '[[DATABASE/spell/Detect Magic|Detect Magic]]'
- '[[DATABASE/spell/Mage Hand|Mage Hand]]'
- '[[DATABASE/spell/Magic Missile|MagicMissile]]'
- '[[DATABASE/spell/Read Aura|Read Aura]]'
- '[[DATABASE/spell/Shield|Shield]]'
- '[[DATABASE/spell/Unseen Servant|Unseen Servant]]'
strength: '+1'
strength_req: '1'
strongest_save:
- Reflex
swim_speed: null
trait:
- '[[DATABASE/trait/Anadi|Anadi]]'
- '[[DATABASE/trait/Humanoid|Humanoid]]'
- '[[DATABASE/trait/Uncommon|Uncommon]]'
type: Creature
vision: null
weakest_save:
- Fortitude
weakness: null
will: '+7'
wisdom: '+2'

---
# Anadi Seeker

Anadi seekers are knowledge seekers who often brave the outside world.
**Recall Knowledge - Humanoid ([[DATABASE/skill/Society|Society]])**: DC 17
**Unspecific Lore**: DC 15
**Specific Lore**: DC 12

# Anadi Seeker<span class="item-type">Creature 1</span>

<span class="trait-uncommon item-trait">Uncommon</span><span class="trait-alignment item-trait">CG</span><span class="trait-size item-trait">Medium</span><span class="item-trait">Anadi</span><span class="item-trait">Humanoid</span>
**Source** [[DATABASE/source/Pathfinder 169. Kindled Magic|Pathfinder #169: Kindled Magic]]
**Perception** +7
**Languages** [[DATABASE/language/Anadi|Anadi]], [[DATABASE/language/Common|Common]]
**Skills** [[DATABASE/skill/Acrobatics|Acrobatics]] +6, [[DATABASE/skill/Arcana|Arcana]] +7, [[DATABASE/skill/Athletics|Athletics]] +4 (+6 to [[DATABASE/action/Climb|Climb]] in hybrid or spider form), [[DATABASE/skill/Crafting|Crafting]] +7 (+11 weaving), [[DATABASE/skill/Nature|Nature]] +5, [[DATABASE/skill/Occultism|Occultism]] +7
**Str** +1, **Dex** +3, **Con** -1, **Int** +4, **Wis** +2, **Cha** +0
**Items** [[DATABASE/weapon/Sling|sling (20 sling bullets)]]

---
**AC** 15; **Fort** +4, **Ref** +10, **Will** +7
**HP** 20

---
**Speed** 25 feet, climb 25 feet
<span class="in-box-ability">**Melee** <span class="action-icon">1</span> fangs +8 [+3/-2] (finesse), **Damage** 1d6+1 piercing plus anadi venom</span><span class="in-box-ability">**Ranged** <span class="action-icon">1</span> sling +8 [+3/-2] (propulsive, range increment 50 feet, reload 1), **Damage** 1d6 bludgeoning</span>**Arcane Innate Spells** DC 17; **1st** _[[DATABASE/spell/Color Spray|color spray]]_, _[[DATABASE/spell/Magic Missile|magic missile]]_, _[[DATABASE/spell/Unseen Servant|unseen servant]]_; **Cantrips** **(1st)** _[[DATABASE/spell/Dancing Lights|dancing lights]]_, _[[DATABASE/spell/Detect Magic|detect magic]]_, _[[DATABASE/spell/Mage Hand|mage hand]]_, _[[DATABASE/spell/Read Aura|read aura]]_, _[[DATABASE/spell/Shield|shield]]_
<span class="in-box-ability">**Anadi Venom** (poison) **Saving Throw** DC 15 Fortitude; **Maximum Duration** 4 rounds; **Stage 1** 1d6 poison damage and [[DATABASE/condition/Flat-Footed|flat-footed]] (1 round); **Stage 2** 1d6 poison damage, flat-footed, and [[DATABASE/condition/Clumsy|clumsy 1]] (1 round)</span><span class="in-box-ability">**Change Shape** <span class="action-icon">1</span> (arcane, concentrate, polymorph, transmutation) The anadi changes into their hybrid, spider, or human form. The above statistics assume the anadi is in hybrid form. While in human form, the anadi seeker loses their climb Speed and can't use their fangs attack. When in spider form, they can't use weapons.</span><span class="in-box-ability">**Spin Silk** (concentrate, exploration, manipulate) By spending several minutes, an anadi can produce silk to craft items made of cloth. An anadi can produce enough silk in a day to craft a single garment.</span>