---
ac: '21'
alignment: LN
all_resistance: null
burrow_speed: null
charisma: '+1'
climb_speed: null
constitution: '+2'
creature_ability:
- Cheek Pouches
- Infused Items
- Quick Bomber
- Quick Stow
- Swarming
creature_family: '[[DATABASE/monsterfamily/Ratfolk|Ratfolk]]'
description: 'Ratfolk grenadiers use alchemy and stealth to defend their communities.<br/><br/><b><u>Recall
  Knowledge - Humanoid</u> ( [[DATABASE/skill/Society|Society]] )</b>: DC 19<br/><b><u>Unspecific
  Lore</u></b>: DC 17<br/><b><u>Specific Lore</u></b>: DC 14'
dexterity: '+4'
element: null
fly_speed: null
fortitude: '+11'
hardness: null
hp: '60'
id: '348'
immunity: null
intelligence: '+4'
land_speed: '25'
language:
- '[[DATABASE/language/Common|Common]]'
level: '4'
max_speed: '25'
name: Ratfolk Grenadier
perception: '+10'
rarity: Common
reflex: '+13'
resistance: null
rus_type_level: null
school: null
sense:
- darkvision
size: Small
skill:
- '[[DATABASE/skill/Acrobatics|Acrobatics]] +9'
- '[[DATABASE/skill/Crafting|Crafting]] +12'
- '[[DATABASE/skill/Deception|Deception]] +7'
- '[[DATABASE/skill/Society|Society]] +10'
- '[[DATABASE/skill/Stealth|Stealth]] +12'
- '[[DATABASE/skill/Thievery|Thievery]] +9'
source: '[[DATABASE/source/Bestiary|Bestiary]]'
speed:
- 25 feet
spell: null
strength: '+0'
strength_req: '0'
strongest_save:
- Reflex
swim_speed: null
trait:
- '[[DATABASE/trait/Humanoid|Humanoid]]'
- '[[DATABASE/trait/Ratfolk|Ratfolk]]'
type: Creature
vision: Darkvision
weakest_save:
- Will
weakness: null
will: '+9'
wisdom: '+2'

---
# Ratfolk Grenadier

Ratfolk grenadiers use alchemy and stealth to defend their communities.
**Recall Knowledge - Humanoid ([[DATABASE/skill/Society|Society]])**: DC 19
**Unspecific Lore**: DC 17
**Specific Lore**: DC 14

# Ratfolk Grenadier<span class="item-type">Creature 4</span>

<span class="trait-alignment item-trait">LN</span><span class="trait-size item-trait">Small</span><span class="item-trait">Humanoid</span><span class="item-trait">Ratfolk</span>
**Source** [[DATABASE/source/Bestiary|Bestiary]]
**Perception** +10; darkvision
**Languages** [[DATABASE/language/Common|Common]]
**Skills** [[DATABASE/skill/Acrobatics|Acrobatics]] +9, [[DATABASE/skill/Crafting|Crafting]] +12, [[DATABASE/skill/Deception|Deception]] +7, [[DATABASE/skill/Society|Society]] +10, [[DATABASE/skill/Stealth|Stealth]] +12, [[DATABASE/skill/Thievery|Thievery]] +9
**Str** +0, **Dex** +4, **Con** +2, **Int** +4, **Wis** +2, **Cha** +1
**Infused Items** A ratfolk grenadier carries 2 infused moderate acid flasks, 2 infused moderate alchemist’s fires, and 2 infused moderate frost vials. These items last for 24 hours, or until the next time they make their daily preparations.
**Items** [[DATABASE/equipment/Alchemist's Tools|alchemist's tools]], [[DATABASE/weapon/Hand Crossbow|hand crossbow (20 bolts)]], [[DATABASE/armor/Studded Leather Armor|studded leather armor]]

---
**AC** 21; **Fort** +11, **Ref** +13, **Will** +9
**HP** 60

---
**Speed** 25 feet
<span class="in-box-ability">**Melee** <span class="action-icon">1</span> fangs +12 [+8/+4] (agile, finesse), **Damage** 1d4 piercing</span><span class="in-box-ability">**Ranged** <span class="action-icon">1</span> hand crossbow +12 [+7/+2] (range increment 60 feet, reload 1), **Damage** 1d6 piercing</span><span class="in-box-ability">**Ranged** <span class="action-icon">1</span> alchemical bomb +13 [+8/+3] (range increment 20 feet, splash), **Effect** varies by bomb</span><span class="in-box-ability">**Cheek Pouches** A ratfolk grenadier has stretchy cheek pouches that can store up to 1 cubic foot of objects (though no more than 4 light items). The ratfolk can remove or store an item using the Interact action. As long as the ratfolk has at least one object in its cheek pouches, its speech is noticeably difficult to understand.</span><span class="in-box-ability">**Quick Bomber** <span class="action-icon">1</span> The ratfolk grenadier draws an alchemical bomb with an Interact action and throws it as a ranged Strike.</span><span class="in-box-ability">**Quick Stow** <span class="action-icon">4</span> **Frequency** once per round. **Effect** The ratfolk grenadier stores one held item of light or negligible Bulk in its cheek pouches.</span><span class="in-box-ability">**Swarming** A ratfolk grenadier can end its movement in the same square as an ally that also has this ability. Only two such creatures can share the same space.</span>