---
ac: '19'
alignment: NE
all_resistance: null
burrow_speed: null
charisma: '+1'
climb_speed: null
constitution: '+1'
creature_ability:
- Lingering Ichor Trigger
- Mob Mentality
- Undead Virulence
creature_family: '[[DATABASE/monsterfamily/Virulak|Virulak]]'
dexterity: '+3'
element: null
fly_speed: null
fortitude: '+10'
hardness: null
hp: 45 ( negative healing )
id: '2056'
immunity:
- '[[DATABASE/trait/Death|death]] effects'
- '[[DATABASE/trait/Disease|disease]]'
- '[[DATABASE/condition/Paralyzed|paralyzed]]'
- '[[DATABASE/trait/Poison|poison]]'
- '[[DATABASE/condition/Unconscious|unconscious]]'
intelligence: '+0'
land_speed: '25'
language:
- '[[DATABASE/language/Common|Common]]'
- '[[DATABASE/language/Necril|Necril]]'
level: '3'
max_speed: '25'
name: Virulak Villager
perception: '+8'
rarity: Uncommon
reflex: '+10'
resistance: null
rus_type_level: null
school: null
sense:
- '[[DATABASE/monsterability/Darkvision|darkvision]]'
size: Medium
skill:
- '[[DATABASE/skill/Athletics|Athletics]] +9'
- '[[DATABASE/skill/Crafting|Crafting]] +7'
- '[[DATABASE/skill/Intimidation|Intimidation]] +8'
- '[[DATABASE/skill/Stealth|Stealth]] +10'
source: '[[DATABASE/source/Pathfinder 182. Graveclaw|Pathfinder #182: Graveclaw]]'
speed:
- 25 feet
spell: null
strength: '+4'
strength_req: '4'
strongest_save:
- Fortitude
- Reflex
swim_speed: null
trait:
- '[[DATABASE/trait/Uncommon|Uncommon]]'
- '[[DATABASE/trait/Undead|Undead]]'
type: Creature
vision: Darkvision
weakest_save:
- Will
weakness: null
will: '+8'
wisdom: '+1'

---
# Virulak Villager

A village of commoners raised to undeath by a mass poisoning might continue to go about the settled routines of life, posing an eerie scene for living creatures who enter their village. Virulak villagers prefer to attack with an advantage of numbers, but even a lone villager can pose a problem for unaware or inexperienced travelers.
**Recall Knowledge - Undead ([[DATABASE/skill/Religion|Religion]])**: DC 20
**Unspecific Lore**: DC 18
**Specific Lore**: DC 15

# Virulak Villager<span class="item-type">Creature 3</span>

<span class="trait-uncommon item-trait">Uncommon</span><span class="trait-alignment item-trait">NE</span><span class="trait-size item-trait">Medium</span><span class="item-trait">Undead</span>
**Source** [[DATABASE/source/Pathfinder 182. Graveclaw|Pathfinder #182: Graveclaw]]
**Perception** +8; [[DATABASE/monsterability/Darkvision|darkvision]]
**Languages** [[DATABASE/language/Common|Common]], [[DATABASE/language/Necril|Necril]]
**Skills** [[DATABASE/skill/Athletics|Athletics]] +9, [[DATABASE/skill/Crafting|Crafting]] +7, [[DATABASE/skill/Intimidation|Intimidation]] +8, [[DATABASE/skill/Stealth|Stealth]] +10
**Str** +4, **Dex** +3, **Con** +1, **Int** +0, **Wis** +1, **Cha** +1

---
**AC** 19; **Fort** +10, **Ref** +10, **Will** +8
**HP** 45 ([[DATABASE/monsterability/Negative Healing|negative healing]]); **Immunities** death effects, disease, [[DATABASE/condition/Paralyzed|paralyzed]], poison, [[DATABASE/condition/Unconscious|unconscious]]
<span class="in-box-ability">**Mob Mentality** As long as the virulak villager and their allies aren't outnumbered by enemies, the virulak villager is immune to fear.</span><span class="in-box-ability">**Lingering Ichor <span class="action-icon">5</span> **Trigger** The virulak takes piercing or slashing damage from a melee Strike** **Effect** The virulak squirts a jet of poisonous ichor from the wound. The creature making the triggering Strike takes 3 [[DATABASE/condition/Persistent Damage|persistent poison damage]] equal to the virulak's level.</span>

---
**Speed** 25 feet
<span class="in-box-ability">**Melee** <span class="action-icon">1</span> claw +12 [+8/+4] (agile), **Damage** 1d8+4 slashing plus [[DATABASE/monsterability/Grab|Grab]] and undead virulence</span><span class="in-box-ability">**Undead Virulence** (poison, virulent) **Saving Throw** DC 19 Fortitude; **Maximum Duration** 4 rounds; **Stage 1** 1d10 poison damage (1 round); **Stage 2** 1d12 poison damage (1 round); **Stage 3** 2d10 poison damage (1 round)</span>

###  The Mob's Spoils

Virulak villagers tear apart those who fall prey to their ruse of an ordinary village at work and consume the remains. They don't generally have much use for their victim's valuables and often stash them somewhere, such as in a root cellar or hayloft. A stealthy and canny infiltrator might discover a pile of blood-soaked valuables and escape without alerting the villagers.