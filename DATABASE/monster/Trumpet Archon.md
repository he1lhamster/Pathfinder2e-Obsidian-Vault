---
ac: '36'
alignment: LG
charisma: '+8'
constitution: '+5'
creature_ability:
- Archon's Door
- Kind Word
- Retributive Strike
- Trumpet Blast
creature_family: '[[DATABASE/monsterfamily/Archon|Archon]]'
dexterity: '+5'
fly_speed: '60'
fortitude: '+24'
hp: '285'
id: '552'
intelligence: '+3'
land_speed: '35'
language:
- '[[DATABASE/language/Celestial|Celestial]]'
- '[[DATABASE/language/Draconic|Draconic]]'
- '[[DATABASE/language/Infernal|Infernal]] ; [[DATABASE/spell/Tongues|tongues]]'
level: '14'
max_speed: '60'
name: Trumpet Archon
perception: '+26'
rarity: Common
reflex: '+24'
resistance:
- sonic 15
sense:
- '[[DATABASE/monsterability/Darkvision|darkvision]]'
size: Medium
skill:
- '[[DATABASE/skill/Acrobatics|Acrobatics]] +25'
- '[[DATABASE/skill/Diplomacy|Diplomacy]] +29'
- '[[DATABASE/skill/Intimidation|Intimidation]] +28'
- '[[DATABASE/skill/Nature|Nature]] +23'
- '[[DATABASE/skill/Religion|Religion]] +23'
- '[[DATABASE/skill/Performance|Performance]] +31'
- '[[DATABASE/skill/Stealth|Stealth]] +25'
source: '[[DATABASE/source/Bestiary 2|Bestiary 2]]'
speed:
- 35 feet
- fly 60 feet
spell:
- '[[DATABASE/spell/Banishment|Banishment]]'
- '[[DATABASE/spell/Breath of Life|Breath ofLife]]'
- '[[DATABASE/spell/Circle of Protection|Circle of Protection]]'
- '[[DATABASE/spell/Dimension Door|Dimension Door]]'
- '[[DATABASE/spell/Heal|Heal]]'
- '[[DATABASE/spell/Heroism|Heroism]]'
- '[[DATABASE/spell/Message|Message]]'
- '[[DATABASE/spell/Sound Burst|SoundBurst]]'
- '[[DATABASE/spell/Tongues|Tongues]]'
- '[[DATABASE/spell/Zealous Conviction|Zealous Conviction]]'
strength: '+7'
strength_req: '7'
strongest_save:
- Will
trait:
- '[[DATABASE/trait/Archon|Archon]]'
- '[[DATABASE/trait/Celestial|Celestial]]'
type: Creature
vision: Darkvision
weakest_save:
- Fortitude
- Reflex
weakness:
- evil 15
will: '+27'
wisdom: '+6'

---
# Trumpet Archon

Trumpet archons are messengers, emissaries, and storytellers—embodiments of the virtue of kindness. They use stories and allegories to prevent bloodshed, and among all archons they are the most likely to work together with angels, perhaps leading to the similarities in their forms. When diplomacy is impossible, trumpet archons rally the righteous to battle—one reassuring glance from a trumpet archon is often all it takes to instill a shaken ally with resounding courage once again.
**Recall Knowledge - Celestial ([[DATABASE/skill/Religion|Religion]])**: DC 32
**Unspecific Lore**: DC 30
**Specific Lore**: DC 27

# Trumpet Archon<span class="item-type">Creature 14</span>

<span class="trait-alignment item-trait">LG</span><span class="trait-size item-trait">Medium</span><span class="item-trait">Archon</span><span class="item-trait">Celestial</span>
**Source** [[DATABASE/source/Bestiary 2|Bestiary 2]] 
**Perception** +26; [[DATABASE/monsterability/Darkvision|darkvision]]
**Languages** [[DATABASE/language/Celestial|Celestial]], [[DATABASE/language/Draconic|Draconic]], [[DATABASE/language/Infernal|Infernal]]; [[DATABASE/spell/Tongues|tongues]]
**Skills** [[DATABASE/skill/Acrobatics|Acrobatics]] +25, [[DATABASE/skill/Diplomacy|Diplomacy]] +29, [[DATABASE/skill/Intimidation|Intimidation]] +28, [[DATABASE/skill/Nature|Nature]] +23, [[DATABASE/skill/Performance|Performance]] +31, [[DATABASE/skill/Religion|Religion]] +23, [[DATABASE/skill/Stealth|Stealth]] +25
**Str** +7, **Dex** +5, **Con** +5, **Int** +3, **Wis** +6, **Cha** +8
**Items** _+1 [[DATABASE/equipment/Resilient|resilient]] [[DATABASE/armor/Full Plate|full plate]]_, _+2 [[DATABASE/equipment/Striking|striking]] [[DATABASE/weapon/Bastard Sword|bastard sword]]_, [[DATABASE/equipment/Musical Instrument|virtuoso trumpet]]

---
**AC** 36; **Fort** +24, **Ref** +24, **Will** +27; +1 status to all saves vs. magic
**HP** 285; **Resistances** sonic 15; **Weaknesses** evil 15
<span class="in-box-ability">**[[DATABASE/monsterability/Retributive Strike|Retributive Strike]] <span class="action-icon">5</span> ** </span>

---
**Speed** 35 feet, fly 60 feet
<span class="in-box-ability">**Melee** <span class="action-icon">1</span> bastard sword +29 [+24/+19] (good, magical, versatile P), **Damage** 2d12+13 slashing plus 2d6 sonic and 1d6 good</span>**Divine Innate Spells** DC 34; **7th** _[[DATABASE/spell/Heal|heal]]_ (×2), _[[DATABASE/spell/Sound Burst|sound burst]]_; **6th** _[[DATABASE/spell/Heroism|heroism]]_, _[[DATABASE/spell/Zealous Conviction|zealous conviction]]_; **5th** _[[DATABASE/spell/Banishment|banishment]]_, _[[DATABASE/spell/Breath of Life|breath of life]]_; **4th** _[[DATABASE/spell/Dimension Door|dimension door]]_ (at will); **3rd** _[[DATABASE/spell/Circle of Protection|circle of protection]]_ (against evil only; ×2); **Cantrips** **(6th)** _[[DATABASE/spell/Message|message]]_; **Constant** **(5th)** _[[DATABASE/spell/Tongues|tongues]]_
<span class="in-box-ability">**Archon's Door** Once per day, if an archon sees another creature cast [[DATABASE/spell/Dimension Door|dimension door]], the archon can use dimension door (heightened to 5th level) within 1 round to attempt to follow that creature to the maximum distance of the archon's dimension door. If the archon's dimension door has enough distance, the archon appears the same distance and direction from the creature as before either creature used dimension door.</span><span class="in-box-ability">**Kind Word** <span class="action-icon">1</span> (auditory, divine, emotion, enchantment, linguistic, mental) The trumpet archon offers an ally praise or encouragement. The archon attempts to counteract one emotion effect on the ally (with a counteract modifier of +24), and the archon and the ally both gain a +1 status bonus to attack rolls and saving throws for 1 round.</span><span class="in-box-ability">**Trumpet Blast** <span class="action-icon">1</span> (auditory, divine, emotion, enchantment, mental) The trumpet archon blows mightily on their horn, creating a tone of such beauty and grandeur those who hear it are paralyzed in awe. Non-archons within 100 feet must attempt a DC 34 Fortitude save. They are temporarily immune for 10 minutes. 
**Critical Success** The creature is unaffected. 
**Success** The creature is [[DATABASE/condition/Flat-Footed|flat-footed]]. 
**Failure** The creature is [[DATABASE/condition/Stunned|stunned 1]] and flat-footed as long as it's stunned. 
**Critical Failure** The creature is [[DATABASE/condition/Paralyzed|paralyzed]] for 1 round.</span>

###  Instruments of War

A trumpet archon's magical instrument is no mere piece of brass; each of these immaculately crafted horns is said to be crafted in Heaven's blessed forges and infused with the voices of a thousand angels. Only archons can truly play such trumpets—mortals lack the power necessary to create trumpet archons' iconic clarion calls, but can still play them as virtuoso instruments.