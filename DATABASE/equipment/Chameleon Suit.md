---
ac: null
actions: '[two-actions]'
alignment: null
base_item: null
bulk: L
burrow_speed: null
climb_speed: null
damage: null
deity: null
duration: null
element: null
favored_weapon: null
fly_speed: null
fortitude: null
frequency: null
hands: null
hardness: null
hp: null
id: '1106'
item_category: Consumables
item_subcategory: Gadgets
land_speed: null
level: '12'
max_speed: null
name: Chameleon Suit
onset: null
price: 320 gp
range: null
rarity: Uncommon
reflex: null
requirement: null
resistance: null
rus_type_level: null
saving_throw: null
school: null
size: null
source: '[[DATABASE/source/Guns & Gears|Guns & Gears]]'
spell: null
stage: null
subcategory: consumable/gadget
swim_speed: null
trait:
- '[[DATABASE/trait/Clockwork|Clockwork]]'
- '[[DATABASE/trait/Consumable|Consumable]]'
- '[[DATABASE/trait/Gadget|Gadget]]'
- '[[DATABASE/trait/Uncommon|Uncommon]]'
trigger: null
type: Item
usage: worn over armor and clothes
weapon_category: null
weapon_group: null
weapon_type: null

---
# Chameleon Suit<span class="item-type">Item 4+</span>

<span class="trait-uncommon item-trait">Uncommon</span><span class="item-trait">Clockwork</span><span class="item-trait">Consumable</span><span class="item-trait">Gadget</span>
**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Usage** worn over armor and clothes; **Bulk** L
**Access** Characters from Absalom, New Thassilon, Alkenstar, Dongun Hold, or especially technological regions of Arcadia like the Deadshot Lands have access to uncommon clockwork items from the first two chapters of this book. Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets.
**Activate** <span class="action-icon">2</span> Interact

---
This complex clockwork outerwear, worn over armor or other clothing, is composed of thin, interlocking scales of myriad colors attached to numerous clockwork gears that can rotate between all visible colors and shades. 
When activated, the clockwork gears work double-time to temporarily switch and adjust the scales to match the exact coloration of the area around you, allowing you to blend in as long as you stay still. For the duration, you can [[DATABASE/action/Hide|Hide]] without needing cover or concealment to do so. This doesn't allow you to [[DATABASE/action/Sneak|Sneak]] without ending your movement in cover or concealment, however, as the clockwork flips over and adjusts the scales to match your background as you move, giving the impression of rippling waves of color and revealing your movement. Once the duration expires, the overworked clockwork and scales fall apart.

## Chameleon Suit<span class="item-type">Item 4</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 16 gp
**Bulk** L
The effect lasts for 1 minute.

## Chameleon Suit (Greater)<span class="item-type">Item 8</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 80 gp
**Bulk** L
The effect lasts for 10 minutes.

## Chameleon Suit (Major)<span class="item-type">Item 12</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 320 gp
**Bulk** L
The effect lasts for 1 hour.