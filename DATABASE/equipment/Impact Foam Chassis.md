---
bulk: L
id: '1115'
item_category: Consumables
item_subcategory: Gadgets
level: '4'
name: Impact Foam Chassis
price: 15 gp
rarity: Uncommon
source: '[[DATABASE/source/Guns & Gears|Guns & Gears]]'
subcategory: consumable/gadget
trait:
- '[[DATABASE/trait/Consumable|Consumable]]'
- '[[DATABASE/trait/Gadget|Gadget]]'
- '[[DATABASE/trait/Uncommon|Uncommon]]'
type: Item
usage: worn over armor and clothes

---
# Impact Foam Chassis<span class="item-type">Item 1+</span>

<span class="trait-uncommon item-trait">Uncommon</span><span class="item-trait">Consumable</span><span class="item-trait">Gadget</span>
**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Usage** worn over armor and clothes; **Bulk** L
**Access** Due to the use of technology, all gadgets are uncommon or rare, though inventors and regions with access to inventors have access to uncommon gadgets.
**Activate** see below

---
While you're wearing an impact foam chassis wrapped around your body, it Activates automatically whenever you fall at least 10 feet, causing a layer of soft, flexible impact foam to expand beneath and around you to cushion the fall. The impact foam decreases the falling damage by up to the listed amount, taking damage equal to the amount of falling damage the foam reduced. If this is enough to destroy the foam, it disperses immediately. However, if the foam has any Hit Points remaining, it remains surrounding you for up to 1 minute. While you're surrounded in foam, you are [[DATABASE/condition/Immobilized|immobilized]] and have standard cover against other creatures, and other creatures have standard cover against you. The foam has an [[DATABASE/action/Escape|Escape]] DC of 15; a creature can also remove it by spending three Interact actions. You or others can also attack the foam (the foam has an AC of 12), which disperses if reduced to 0 Hit Points.

## Impact Foam Chassis (Lesser)<span class="item-type">Item 1</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 3 gp
**Bulk** L
A lesser impact foam chassis prevents up to 20 falling damage and has 20 Hit Points.

## Impact Foam Chassis (Moderate)<span class="item-type">Item 4</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 15 gp
**Bulk** L
A moderate impact foam chassis prevents up to 50 falling damage and has 40 Hit Points.

## Impact Foam Chassis (Greater)<span class="item-type">Item 9</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 150 gp
**Bulk** L
A greater impact foam chassis prevents up to 100 falling damage and has 100 Hit Points.

## Impact Foam Chassis (Major)<span class="item-type">Item 14</span>

**Source** [[DATABASE/source/Guns & Gears|Guns & Gears]]
**Price** 650 gp
**Bulk** L
A major impact foam chassis prevents up to 200 falling damage and has 200 Hit Points.