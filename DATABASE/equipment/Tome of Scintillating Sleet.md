---
ac: null
actions: null
alignment: null
base_item: null
bulk: L
burrow_speed: null
climb_speed: null
damage: null
deity: null
duration: null
element: null
favored_weapon: null
fly_speed: null
fortitude: null
frequency: null
hands: null
hardness: null
hp: null
id: '2184'
item_category: Grimoires
item_subcategory: null
land_speed: null
level: '8'
max_speed: null
name: Tome of Scintillating Sleet
onset: null
price: 430 gp
range: null
rarity: Common
reflex: null
requirement: null
resistance: null
saving_throw: null
school: Evocation
size: null
source: '[[DATABASE/source/Treasure Vault|Treasure Vault]]'
spell: null
stage: null
subcategory: grimoire
swim_speed: null
trait:
- '[[DATABASE/trait/Cold|Cold]]'
- '[[DATABASE/trait/Evocation|Evocation]]'
- '[[DATABASE/trait/Grimoire|Grimoire]]'
- '[[DATABASE/trait/Primal|Primal]]'
trigger: null
type: Item
usage: null
weapon_category: null
weapon_group: null
weapon_type: null

---
# Tome of Scintillating Sleet<span class="item-type">Item 8</span>

<span class="item-trait">Cold</span><span class="item-trait">Evocation</span><span class="item-trait">Grimoire</span><span class="item-trait">Primal</span>
**Source** [[Treasure Vault]] 
**Price** 430 gp
**Bulk** L

---
The first 12 pages of this tome tell the same story in two languages: 6 pages in [[Skald]] and 6 pages in the ancient [[Jotun]] dialect used by saumen kar, a species of ice-dwelling humanoids. The story is a tale of a saumen kar stricken with snow blindness after spending too long under the sun building snow giants.
**Activate** <span class="action-icon">4</span> envision **Frequency** once per day; **Effect** If your next action is to cast a [[Cold Domain]] spell that deals damage, all creatures damaged by the spell are also [[Dazzled]] for 3 rounds by light refracting and reflecting within and around the spell's chilling effects. If an affected creature critically failed its save against the required spell, or if you critically succeeded on your spell attack roll against the creature, it's instead [[Blinded]] for 1 round and then dazzled for 3 rounds.