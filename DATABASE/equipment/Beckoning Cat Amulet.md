---
actions: '[free-action]'
id: '582'
item_category: Consumables
item_subcategory: Talismans
level: '5'
name: Beckoning Cat Amulet
price: 26 gp
rarity: Uncommon
school: Divination
source: '[[DATABASE/source/Pathfinder 152. Legacy of the Lost God|Pathfinder #152:
  Legacy of the Lost God]]'
subcategory: consumable/talisman
trait:
- '[[DATABASE/trait/Consumable|Consumable]]'
- '[[DATABASE/trait/Divination|Divination]]'
- '[[DATABASE/trait/Fortune|Fortune]]'
- '[[DATABASE/trait/Magical|Magical]]'
- '[[DATABASE/trait/Talisman|Talisman]]'
- '[[DATABASE/trait/Uncommon|Uncommon]]'
trigger: "You use a feat or ability to reroll a failed or critically failed Reflex\
  \ saving throw (such as [[DATABASE/monster/Catfolk Pouncer|Cat\u2019s Luck]] or\
  \ [[DATABASE/feat/Halfling Luck|Halfling Luck]] )."
type: Item
usage: affixed to armor

---
# Beckoning Cat Amulet<span class="item-type">Item 5</span>

<span class="trait-uncommon item-trait">Uncommon</span><span class="item-trait">Consumable</span><span class="item-trait">Divination</span><span class="item-trait">Fortune</span><span class="item-trait">Magical</span><span class="item-trait">Talisman</span>
**Source** [[DATABASE/source/Pathfinder 152. Legacy of the Lost God|Pathfinder #152: Legacy of the Lost God]]
**Price** 26 gp
**Usage** affixed to armor
; **Bulk** —
**Activate** <span class="action-icon">4</span> envision **Trigger** You use a feat or ability to reroll a failed or critically failed Reflex saving throw (such as [[DATABASE/monster/Catfolk Pouncer|Cat’s Luck]] or [[DATABASE/feat/Halfling Luck|Halfling Luck]]).

---
This clay figurine resembles a white cat with a paw outstretched. When it’s activated, any allies within 10 feet who also just failed or critically failed a Reflex saving throw from the same source (such as a [[DATABASE/spell/Fireball|fireball]] spell) can also reroll their saving throw and use the better result.