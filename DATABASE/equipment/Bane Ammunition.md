---
ac: null
actions: '[one-action]'
alignment: null
base_item: null
bulk: null
burrow_speed: null
climb_speed: null
damage: null
deity: null
duration: null
element: null
favored_weapon: null
fly_speed: null
fortitude: null
frequency: null
hands: null
hardness: null
hp: null
id: '1895'
item_category: Alchemical Items
item_subcategory: Alchemical Ammunition
land_speed: null
level: '11'
max_speed: null
name: Bane Ammunition
onset: null
price: 250 gp
range: null
rarity: Common
reflex: null
requirement: null
resistance: null
rus_type_level: null
saving_throw: null
school: null
size: null
source: '[[DATABASE/source/Treasure Vault|Treasure Vault]]'
spell: null
stage: null
subcategory: alchemical/ammunition
swim_speed: null
trait:
- '[[DATABASE/trait/Alchemical|Alchemical]]'
- '[[DATABASE/trait/Consumable|Consumable]]'
- '[[DATABASE/trait/Poison|Poison]]'
trigger: null
type: Item
usage: null
weapon_category: null
weapon_group: null
weapon_type: null

---
# Bane Ammunition<span class="item-type">Item 1+</span>

<span class="item-trait">Alchemical</span><span class="item-trait">Consumable</span><span class="item-trait">Poison</span>
**Source** [[DATABASE/source/Treasure Vault|Treasure Vault]] 
**Ammunition** any; **Bulk** —
**Activate** <span class="action-icon">1</span> Interact

---
Monster hunters favor bane ammunition that contains a capsule of reagents tailored to a particular type of creature— aberration, animal, beast, dragon, fey, giant, ooze, or both fungus and plant. Each type requires a different formula. When activated bane ammunition hits a target that has a trait matching the selected type, it takes [[DATABASE/condition/Persistent Damage|persistent poison damage]] in addition to the damage the attack normally deals.

## Bane Ammunition (Lesser)<span class="item-type">Item 1</span>

**Source** [[DATABASE/source/Treasure Vault|Treasure Vault]] 
**Price** 3 gp
The ammunition deals 1d4 persistent poison damage.

## Bane Ammunition (Moderate)<span class="item-type">Item 5</span>

**Source** [[DATABASE/source/Treasure Vault|Treasure Vault]] 
**Price** 25 gp
The ammunition deals 2d6 persistent poison damage.

## Bane Ammunition (Greater)<span class="item-type">Item 11</span>

**Source** [[DATABASE/source/Treasure Vault|Treasure Vault]] 
**Price** 250 gp
The ammunition deals 3d6 persistent poison damage.