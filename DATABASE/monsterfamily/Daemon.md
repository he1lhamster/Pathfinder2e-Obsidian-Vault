---
creature_family: Daemon
id: '26'
name: Daemon
rarity: Common
source: '[[DATABASE/source/Bestiary|Bestiary]]'
type: Creature Family

---
# Daemon

**Source** [[DATABASE/source/Bestiary|Bestiary]]
Denizens of the bleak and terrible plane of Abaddon, daemons are shaped by and devoted to the destruction of life in all its forms. They seek the death of every mortal being by the most painful and horrible means possible, all in service to the apocalyptic entities known as the Four Horsemen. Each kind of daemon represents a different way to die, and their powers are nearly always aimed at spreading that particular form of death. Through the use of these powers, they seek to drag all existence down into a pit of hopelessness and despair, and to commit all souls to oblivion.
 While those who summon daemons to the Material Plane usually seek to use the creatures’ destructive and corrupting powers for their own ends, daemons always look for ways to spread fear, doubt, and despair wherever they go. Often, daemons disguise their plots as the workings of other fiends, knowing that such confusion compounds mortals’ fear.
 While all fiends seek to tempt mortals into lives of evil to increase their own numbers and power on their native planes, daemons are further driven by a supernatural hunger for mortal souls and use a variety of methods—not least of which is the cacodaemons’ soul gems—to entrap them. On Abaddon and in other forbidding places across the multiverse, souls are simultaneously a delicacy, a trade good, and a source of magical power, and the daemons are among the greatest gluttons, merchants, and abusers of this spiritual “resource.”

### Members

[[DATABASE/monster/Agradaemon (Conflagration Daemon)|Agradaemon]] (Creature 19), [[DATABASE/monster/Astradaemon (Void Daemon)|Astradaemon]] (Creature 16), [[DATABASE/monster/Cacodaemon (Harvester Daemon)|Cacodaemon]] (Creature 1), [[DATABASE/monster/Ceustodaemon (Guardian Daemon)|Ceustodaemon]] (Creature 6), [[DATABASE/monster/Crucidaemon (Torture Daemon)|Crucidaemon]] (Creature 15), [[DATABASE/monster/Derghodaemon (Ravager Daemon)|Derghodaemon]] (Creature 12), [[DATABASE/monster/Leukodaemon (Pestilence Daemon)|Leukodaemon]] (Creature 9), [[DATABASE/monster/Meladaemon (Famine Daemon)|Meladaemon]] (Creature 11), [[DATABASE/monster/Obcisidaemon (Obliteration Daemon)|Obcisidaemon]] (Creature 19), [[DATABASE/monster/Olethrodaemon (Apocalypse Daemon)|Olethrodaemon]] (Creature 20), [[DATABASE/monster/Phasmadaemon (Terror Daemon)|Phasmadaemon]] (Creature 17), [[DATABASE/monster/Piscodaemon (Venom Daemon)|Piscodaemon]] (Creature 10), [[DATABASE/monster/Purrodaemon (War Daemon)|Purrodaemon]] (Creature 18), [[DATABASE/monster/Sordesdaemon (Pollution Daemon)|Sordesdaemon]] (Creature 15), [[DATABASE/monster/Thanadaemon (Death Daemon)|Thanadaemon]] (Creature 13)

###  Abaddon

Daemons hail from [[DATABASE/plane/Abaddon|Abaddon]], a plane of pure evil for only the most corrupt souls. If [[DATABASE/plane/Hell|Hell]] is a realm of torture and systematic depravity, Abaddon is its twisted cousin—a place where one can find only misery and despair in ample quantities, with neither the strictures of Hell nor the chaotic freedom of the [[DATABASE/plane/Abyss|Abyss]]. For evildoers who seek simply to inflict their wickedness on others, Abaddon is a rich hunting ground where one can revel in turpitude, though visitors be forewarned: the competition is fierce.

###  Daemon Harbingers

The four Horsemen are the most powerful of the daemonic demigods, but they are only four of many. Far more daemonic harbingers dwell on [[DATABASE/plane/Abaddon|Abaddon]], each a unique and powerful demigod in their own right that enjoys sweeping influence among daemonkind. Jacarkas the Collector (ruler of the slave city of Awaiting Consumption), Vorasha the Ophidian (consort to the Horseman of Famine), and Zelishkar the Bitter Flame (patron to arsonists and agent of the Horseman of War) are but three of the dozens of harbingers who rule realms in Abaddon.

###  Daemonic Deacons

Each of the four Horsemen is served by a specific category of servitor daemons known as deacons. Leukodaemons serve as deacons to the [[DATABASE/deity/Apollyon|Horseman of Pestilence]]. Meladaemons serve as deacons to the [[DATABASE/deity/Trelmarixian|Horseman of Famine]], while thanadaemons serve as deacons to the [[DATABASE/deity/Charon|Horseman of Death]]. Purrodaemons are currently the most powerful of the deacons, and serve the [[DATABASE/deity/Szuriel|Horseman of War]]—yet over time, which category of deacon is the most powerful can wax and wane.

###  Daemonic Divinities

Numerous powerful and unique daemon demigods, known collectively as harbingers, rule over swaths of Abaddon. Above these demigods, though, are entities of even greater power—the four Horsemen of the Apocalypse. As the eons go on, the names and identities of specific horsemen change. Currently, they consist of [[DATABASE/deity/Apollyon|Apollyon]] (Horseman of Pestilence), [[DATABASE/deity/Charon|Charon]] (Horseman of Death), [[DATABASE/deity/Szuriel|Szuriel]] (Horseman of War), and [[DATABASE/deity/Trelmarixian|Trelmarixian]] (Horseman of Famine). Of these, only Charon has never fallen to an upstart. Some hold that a “Fifth Horseman” once ruled over the other four, while others maintain that the eternally eclipsed sun in the skies above Abaddon is all that remains of this long-dead god.

###  Other Daemons

As many daemons exist as there are awful ways to die. The bloody sangudaemon personifies death by blood loss, while the skeletal thanadaemon represents death from old age. The most powerful daemons are the olethrodaemons, who represent the massive deaths caused by apocalypses and the end of entire worlds.

###  Soul Gems as Gear

Daemons often carry soul gems either as trophies or for powering an ability. [[DATABASE/monster/Cacodaemon (Harvester Daemon)|Cacodaemons]] are the most common source for soul gems, but they can also be created by spells like [[DATABASE/spell/Bind Soul|bind soul]]. If a daemon crushes a soul gem to power an ability, the trapped soul is released into the afterlife and can be resurrected normally.

###  Soul Gems as Treasure

Soul gems are traded in illicit markets, a tradition celestials and psychopomps alike find vile. Soul gems' value varies, but is generally worth an amount relative to the level of a gem's captive soul.

###  The Daemonic Paradox

Daemons embody a fundamental paradox—while they are incarnations of death and seek to devour all that lives, they are themselves living creatures. Some speak of a glorious end time after which reality will finally be free of the contagion that is life itself. Most daemons give no thought to this paradox.